...
2. 触发方式选 **定时触发**；Cron 表达式填：
   ...
   0 0 * * * ? *
   ```
   （腾讯云 SCF 的 cron 是 7 段：秒 分 时 日 月 星期 年，上面表示**每小时整点触发**。
   ⚠️ 「日」和「星期」不能同时填 `*`，所以星期用 `?` 表示「不指定」，否则会报 cron invalid。）
3. 确认创建。

> 想改成“每小时第 30 分”触发：`0 30 * * * ? *`。
...
| 每小时都在 push → Pages 每次自动发布 | 正常。公开仓库免费，每小时一次没问题。 |
| 充值时看到一条红色/余额跳升 | 正常。检测到余额上升即判定为充值（如充 100 到账约 99.4，平台已扣 0.6% 手续费），当小时不计入消耗。 |
